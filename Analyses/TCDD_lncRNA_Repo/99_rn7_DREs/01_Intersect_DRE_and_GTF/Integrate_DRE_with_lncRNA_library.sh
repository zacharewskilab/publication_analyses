...
DRE_File='./RAW_Data/rn7_DRE_scored_mss_0.825.txt'
...
### Process DRE File
##############################
awk -F'\t' 'BEGIN {OFS="\t"} NR>1 {print $2, $4, $5, $1, $3, $8}' "$DRE_File" > DREs_Processed1.bed
...
##############################
bedtools intersect -a DREs_Processed1.bed -b GTF_Processed6.gtf -wa -wb -loj > ./Overlapping_Regions_DREs_lncRNAs.bed

# Add headers to results
echo -e "Chromosome\tDRE_Start\tDRE_End\tDRE_ID\tDRE_Strand\tDRE_MSS\tGTF_Chromosome\tGene_Start\tGene_End\tGene_Strand\tGene_ORIG_Start\tMetadata" > temp_file.txt

awk -F'\t' 'NR==1 || $6 >= 0.825' ./Overlapping_Regions_DREs_lncRNAs.bed >> temp_file.txt

mv temp_file.txt ./DREs_in_lncRNAs_MSS_0.825.bed
...
rm DREs_Processed1.bed
rm ./Overlapping_Regions_DREs_lncRNAs.bed
